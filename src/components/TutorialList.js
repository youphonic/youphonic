...
const TutorialList = (props) => {
  ...
  const tutorials = [{
    ...
    text: `Add a bouncing ball. Click pause to set its tone and add a drum beat. While paused, you drag it to a new position, and use the red arrow to set its direction and speed. Particle is the Moving Cirlc's tiny twin`,
...
`,
  ...
  const tutorialItems = tutorials.map((item, index) => {
    return (
      <TutorialListItem
        key={index}
