export const ENTERED_APP = 'ENTERED_APP';
export const START_CANVAS = 'START_CANVAS';
export const STOP_CANVAS = 'STOP_CANVAS';

export const OPEN_LOGIN = 'OPEN_LOGIN';
export const CLOSE_LOGIN = 'CLOSE_LOGIN';
export const OPEN_SIGNUP = 'OPEN_SIGNUP';
export const CLOSE_SIGNUP = 'CLOSE_SIGNUP';
export const OPEN_WINDOW_SETTINGS = 'OPEN_WINDOW_SETTINGS';
export const CLOSE_WINDOW_SETTINGS = 'CLOSE_WINDOW_SETTINGS';
export const OPEN_LOGIN_ALERT = 'OPEN_LOGIN_ALERT';
export const CLOSE_SIGNUP_ALERT = 'CLOSE_SIGNUP_ALERT';
export const OPEN_PLAYS = 'OPEN_PLAYS';
export const CLOSE_PLAYS = 'CLOSE_PLAYS';
export const OPEN_SHAPESETTINGS = 'OPEN_SHAPESETTINGS';
export const CLOSE_SHAPESETTINGS = 'CLOSE_SHAPESETTINGS';

export const GET_ALL_PLAYS = 'GET_ALL_PLAYS';

export const ADD_CHUNK = 'ADD_CHUNK';
export const LOAD_CHUNKS = 'LOAD_CHUNKS';

export const TOGGLE_PLAY = 'TOGGLE_PLAY';

export const SELECT_CHUNK = 'SELECT_CHUNK';
export const REMOVE_CHUNK = 'REMOVE_CHUNK';
export const CLEAR_ALL_CHUNKS = 'CLEAR_ALL_CHUNKS';
export const UPDATE_SELECTED_CHUNK = 'UPDATE_SELECTED_CHUNK';
export const UPDATE_ONE_CHUNK = 'UPDATE_ONE_CHUNK';

export const TOGGLE_SAVE_A_PLAY = 'TOGGLE_SAVE_A_PLAY';

export const SET_GRID = 'SET_GRID';
export const TOGGLE_GRID = 'TOGGLE_GRID';
